...
For each entry, the DNA sequence of each TE-gene and each annotated transcript is reported, together with the protein sequence of the first and the longest ORF on each sense (forward and reverse), the presence of DAPseq peaks (raw data can be found [here](../resources/DAPseq_results.tsv.gz)) and the expression of each TE-gene under different conditions.
